...
def test_team_leaders_computes_opponent_points_per_game_from_plus_minus(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    ...
    assert df.iloc[0]["team_name"] == "Alpha"
    assert df.iloc[0]["opponent_pts_per_game"] == 90
    assert df.iloc[1]["team_name"] == "Beta"
    assert df.iloc[1]["opponent_pts_per_game"] == 105
