...
| Starter / bench role      | `player_game_summary` (verified in Phase E smoke; current supported scope is player-context only)                            | `query_service` forwards `role`; `_route_context_filters_for_execution()` preserves it on the initial Phase G routes and appends `build_role_filter_note()` before command execution                                                 | `player_game_summary.build_result()`                                                                                                     | `data/raw/player_game_stats/*`, which already includes `starter_flag` derived from `start_position` during `pull_player_game_stats.py`                            | No new raw data is required; the remaining gap is command-level filtering on `starter_flag` consistently across the supported player routes                      |
