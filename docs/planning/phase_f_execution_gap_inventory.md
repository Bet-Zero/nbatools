...
| Starter / bench role      | `unfiltered`   | Surface forms: `as a starter`, `starting`, `off the bench`, `bench`, `reserve`. Player-context only; team-only bench phrasing is intentionally ignored. Verified route shape in smoke: `player_game_summary`.                       | A trustworthy per-game starter-role source or derivation joined to the player-summary / player-finder execution paths.                                   | The current player-game corpus does not contain trustworthy starter-role data: `start_position` is empty throughout and the derived `starter_flag` stays `0`, so route-level role filtering would produce false `no_match` results. | Part 2 plan §4; parser spec §8; parser examples §7.10; query catalog §3.11; `src/nbatools/commands/pipeline/pull_player_game_stats.py` |
...
- Required data / aggregation:
  - a trustworthy per-game starter-role source or derivation keyed to the player game logs used by `player_game_summary` and `player_game_finder`
  - command-level support for the player-summary / player-finder family to honor that signal consistently once it exists
- Current blockers:
...
- Starter / bench role:
  - The player routes and transport path are already identified, but the current raw role signal is not trustworthy enough to execute against.
...
- land a trustworthy whole-game starter / bench signal for player-context routes; this may be an ingested field or a derived table, but it cannot be the current all-zero `starter_flag`
- provide join keys at least as strong as `season`, `season_type`, `game_id`, and `player_id` so the role signal can ride on the existing player-game routes
...
- starter / bench execution is now a data-source / derivation problem first and an execution-plumbing problem second

Phase-G implication:

- the next role item has to audit candidate starter-role sources and define this contract before route-level filtering reopens
- once the contract exists, the follow-on execution item can wire role filtering through player summary / finder without guessing at the data source
...
- Later queues should schedule work by prerequisite, not by individual filter phrase. For example: "land the starter-role data contract" or "wire role filtering through player summary and finder once the contract exists" is a queue item; "support `as a starter`" is just the surface expression that rides on that prerequisite.
