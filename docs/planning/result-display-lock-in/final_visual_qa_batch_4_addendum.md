...
Only 18 shapes/screenshots were present in the latest review page. The missing reviewed family appears to be **Playoff History** / standalone team playoff history, previously represented by the Lakers playoff history screenshot.
...
| Missing Playoff History family | Needs investigation | Confirm |
...
## Missing family / shape count issue

The review page now shows 18 shapes instead of 19. Based on the screenshots received across all batches, the missing family appears to be:

- **Playoff History** — standalone team playoff history, e.g. `Lakers playoff history` / Fixture 228 from earlier review.

This is separate from:

- Playoff Matchup History — received in Batch 2
- Playoff Round Records — received in Batch 2

### Follow-up

- Investigate why the review page now groups or omits the standalone Playoff History family.
- Confirm whether Playoff History was intentionally merged into another shape or accidentally dropped from the one-example-per-family review set.
- If still supported, regenerate/review the standalone Playoff History screenshot before final closure.
...
2. **Rolling-stretch context metric label**
   - `Metric: PTS` should become `Metric: PPG` or `Metric: Scoring average` when the visible ranked metric is PPG.

## Confirm

3. **Missing standalone Playoff History family**
   - Review page shows 18 shapes, not 19.
   - Likely missing Family 9: Playoff History / Lakers playoff history / Fixture 228.
   - Determine whether it was intentionally merged or accidentally omitted.
...
16. Investigate missing Playoff History shape and review Fixture 228 if still supported.
