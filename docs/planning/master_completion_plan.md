...
[`phase_m_work_queue.md`](./phase_m_work_queue.md).

The current required action is to work the first unchecked item in
[`phase_m_work_queue.md`](./phase_m_work_queue.md): resolve the remaining
on/off net-rating phrasing mismatch.
...
CI, merge immediately when green, and continue to the next unchecked item unless
the queue explicitly requires review-handoff/blocker escalation.
...
| Examples-library surface | Full-sweep audit exists for `docs/architecture/parser/examples.md`                  | Latest sweep shows 3 remaining failures, 1 phrasing-pair mismatch, and 0 equivalence-group mismatches after Phase L improved the Phase K 384/18 baseline to 399 passing cases and 3 failing cases                                                                                                               | Not complete until failures are fixed in behavior or honestly reclassified in docs, and the full-sweep protocol shows no unresolved canonical/example-library blockers                         | Not done                  | [`parser_examples_completion_plan.md`](./parser_examples_completion_plan.md) / [`phase_m_work_queue.md`](./phase_m_work_queue.md) |
...
- [`phase_m_work_queue.md`](./phase_m_work_queue.md) is the active queue for
  the remaining parser-examples blockers from the fresh 399/3 sweep. Agents
  must work the first unchecked item there before moving to later Phase M
  items.
