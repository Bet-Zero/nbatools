...
> Role: audit output for item 1 in [phase_f_work_queue.md](../completed-work-queues/phase_f_work_queue.md).
...
| On/off                    | `execution-backed with coverage gate` | Surface forms: `on/off`, `with X on the floor`, `without X on the floor`, `X on court`, `X off court`, `X sitting`. Dedicated route: `player_on_off`.                                                                               | `team_player_on_off_summary`, normalized from upstream `teamplayeronoffsummary` via `nba_api.stats.endpoints.TeamPlayerOnOffSummary`.                                           | Execution works for supported single-player slices when trusted on/off coverage exists. Missing/untrusted coverage and multi-player on/off requests keep explicit unsupported/no-result responses; `without_player` remains a whole-game absence filter, not an on/off mechanism. | Part 2 plan §4; parser spec §11; parser examples §7.15; query catalog §2.3 / §3.3; Phase E data inventory; Phase I source boundary; data contracts §1D |
| Lineups                   | `placeholder; source approved` | Surface forms: `best 5-man lineups`, `3-man units`, `2-man combos`, `lineup with X and Y`, `with X and Y together`. Dedicated routes: `lineup_summary`, `lineup_leaderboard`.                                                       | Approved future path: upstream `leaguelineupviz` via `nba_api.stats.endpoints.LeagueLineupViz`, documented in `phase_j_lineup_source_boundary.md`.                                  | No `league_lineup_viz` dataset or coverage-gated route execution exists yet. Placeholder routes remain honest; roster snapshots are season membership only and cannot reconstruct unit-level stints.                                                           | Part 2 plan §4; parser spec §11; parser examples §§7.16-7.17; query catalog §2.3 / §3.3; Phase E data inventory; Phase J source boundary; data contracts §1E |
...
- Historical blockers / deferral boundary:
  ...
  - later master-plan queues implemented coverage-gated clutch execution for
    ...
    controlled by [`master_completion_plan.md`](../completed-plans/master_completion_plan.md)
...
- Current execution behavior:
  ...
  - route execution uses trusted `team_player_on_off_summary` rows when coverage
    exists
- Required data / aggregation:
  - upstream `teamplayeronoffsummary` rows normalized into
    `team_player_on_off_summary`
- Current blockers / deferral boundary:
  - missing or untrusted `team_player_on_off_summary` coverage keeps the
    explicit unsupported-data response
  - multi-player on/off and stint-level semantics remain outside the current
    source contract
  - the existing `without_player` helper excludes whole games and cannot answer possession-level on/off questions
  - see [`phase_i_on_off_source_boundary.md`](../handoffs-and-boundaries/phase_i_on_off_source_boundary.md)
...
- Current execution behavior:
  ...
  - the source path is approved, but execution remains deferred until ingestion,
    validation, and coverage-gated route execution are implemented
- Required data / aggregation:
  - upstream `leaguelineupviz` rows normalized into `league_lineup_viz`
  - minute-threshold support tied to the source `MinutesMin` parameter
- Current blockers / deferral boundary:
  - no raw or processed `league_lineup_viz` dataset exists
  ...
  - see [`phase_j_lineup_source_boundary.md`](../handoffs-and-boundaries/phase_j_lineup_source_boundary.md)
    for the exact future source requirements
...
| On/off                    | `player_on_off`                                                                                                               | `query_service` forwards `lineup_members` and `presence_state`; `_execute_build_result()` dispatches to coverage-gated `player_on_off` execution                                                                                         | `player_on_off.build_result()`                                                                                                           | `team_player_on_off_summary` from upstream `teamplayeronoffsummary`                                                                                                                                        | Missing/untrusted coverage keeps explicit unsupported/no-result responses; multi-player/stint semantics remain out of scope for this boundary                            |
| Lineups                   | `lineup_summary`, `lineup_leaderboard`                                                                                        | `query_service` forwards `lineup_members`, `unit_size`, `minute_minimum`, and `presence_state`; `_execute_build_result()` dispatches straight to placeholder lineup routes until the approved source is ingested and coverage-gated execution is implemented | `lineup_summary.build_result()`, `lineup_leaderboard.build_result()`                                                                     | No `league_lineup_viz` dataset exists yet; roster snapshots are season membership only                                                                                                                       | Approved `leaguelineupviz` source path still needs ingestion, validation, loader support, and command-level lineup execution                                             |
