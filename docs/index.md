...
- `planning/raw-product/RAW_PRODUCT_QA_RELEASE_READINESS_CHECKPOINT.md` — release-readiness checkpoint for backend raw QA, frontend-copy QA, visual QA, public UI readiness, deploy parity, post-review hardening closure, and next options
- `planning/raw-product/RAW_PRODUCT_RELEASE_READINESS_CHECKLIST.md` — current Raw Product release-readiness checklist, preview manual validation steps, final public UI readiness evidence, and post-review hardening closure notes
- `planning/raw-product/RAW_PRODUCT_RELEASE_PACKAGE.md` — final release package for the current supported and explicitly unsupported Raw Product boundary; current public UI status is ready with notes and post-review hardening Waves 1–6 are complete
- `planning/raw-product/RAW_PRODUCT_RELEASE_CANDIDATE_HANDOFF.md` — final release-candidate handoff for the current Raw Product boundary; current state is handoff complete with public UI ready with notes and full frontend tests clean
