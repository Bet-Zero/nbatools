...
answers HTTP `410 Gone`. The monitor correctly reports that as a failure, but
the failure describes the target rather than the service: the application can
be serving normally on the alias at the same time. That is what happened to the
retired `nbatools-fvdbt0pfv-brents-projects-686e97fc.vercel.app` target, which
produced a sustained sequence of scheduled monitor failures.

A dead target makes the monitor uninformative in both directions. It reports
failure regardless of service state, so while it is dead the schedule proves
nothing about availability — neither that the service was down nor that it was
up. Recovering the target restores the signal going forward; it does not
retroactively establish what the service was doing during the affected window.

Historical acceptance receipts still name the deployment URL that was live when
they were signed; those are evidence of a past state and are not rewritten when
the alias later points somewhere else.
